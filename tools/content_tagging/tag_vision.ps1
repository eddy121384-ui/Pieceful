...
foreach($e in @($manifest.entries)){
  ...
  }catch{$c=Clone $e;$c|Add-Member -Force vision_tagging ([pscustomobject]@{status="failed";review_required=$true;review_reasons=@("tagging_error");rationale=$_.Exception.Message;generated_at=(UtcNow)});Put $c;Save;Write-Warning "$($e.id): $($_.Exception.Message)"}
